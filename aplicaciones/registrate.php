...
if( isset($_POST['send'])) {
  ...
  if(  $password1 != "error" && $password2 != "error" && $email1 != "error"){
    #para evaluar si ya tenemos registrado el correo a ingresar
        ...
        echo "<div style='position:fixed; top:140px; left:100px; z-index: 4; width:48%; height:20%; margin:auto;'>";
          echo "<small class='msj_creando_usuario'>ATENCION!! El correo <b> $correo </b> ya existe</small> ";
          echo "</br>";
      ...
      if ( $cantidad_de_publicaciones > 0 ) {
        for ( $contador = 0 ; $contador < $cantidad_de_publicaciones ; $contador ++ ) {
            ( $contador + 1 );
            #$nombres = permimitir_comilla_simple_nombres ($nombres);
            #$apellidos = permimitir_comilla_simple_apellidos ($apellidos);
        ...
        mail ( $CORREO_ADMIN, "Nuevo miembro en el $NOMBRE_BMU ", "$nombres $apellidos ha creado una cuenta, correo corresponde a $correo");
        mail ( $correo, "BIENVENIDO A $NOMBRE_BMU ", "$nombres $apellidos en hora buena eres miembro de $NOMBRE_BMU, comparte conocimiento!");
      }
        echo "<div style='position:fixed; top:140px; left:100px; z-index: 4; width:48%; height:20%; margin:auto;'>";
          echo "<form action='./index.php' method='post'>";
            echo"<small class='msj_creando_usuario'> Tu cuenta se ha creado!. Bienvenido/a <b>$nombres</b> :D </br></small>";
    ...
    echo "
      <div style='position:fixed; top:110px; left:-80px; z-index: 4; width:48%; height:20%; margin:auto;'>
         <h4>Crear Cuenta</h4>
         <form role='form' id='form1' action='./' method='post'>
         <table border='0' width='40%' bordercolor=''>
             ...
             <tr align='center'>
               <td>
                 <input tabindex='1' name='name' id='name' type='text' placeholder='NOMBRES' class='regis $name' value='' />
               </td>
               <td>
                 <input tabindex='2' name='apellid' id='apellid' type='text' placeholder='APELLIDOS' class='regis $apellid' value='' />
               </td>
             </tr>
    ...
    echo "
      <div style='position:fixed; top:180px; left:-80px; z-index: 4; width:48%; height:20%; margin:auto;'>

         <h4>Crear Cuenta</h4>
         <form role='form' id='form1' action='./' method='post'>
         <table border='0' align='' width='40%' bordercolor=''>
             ...
             <tr align='center'>
               <td colspan='2'>
                 ...
                 <span style=color:#6282C1 class='ayuda'></span>                
                 <input tabindex='4' title='8 caracteres como mínimo' name='password1' id='password1' type='password' placeholder='CLAVE' class='regis $password1' value='' />
               </td>
             </tr>
             <tr align='center'>
               <td colspan='2'>
                 ...
                 <button tabindex='6'class='cuenta c-default' >Listo</button><input  name='send' id='send' type='hidden'>
                 </form>
               </td>
             </tr>
         </table>
...
#                 <input tabindex='6' name='send' id='send' type='submit' class='submit' value='Crear Cuenta' />
?>
